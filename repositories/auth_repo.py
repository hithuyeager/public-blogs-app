...
async def replace_refresh_token(conn: asyncpg.Connection,user_id: UUID,hashed_refresh_token: str) -> str:
    status = await conn.fetchval("UPDATE TABLE user_session SET hash_refresh_token = $2 WHERE user_id = $1 RETURNING id",user_id,hashed_refresh_token)
    return str(status)
async def make_user_active(conn: asyncpg.Connection,user_id: UUID) -> str:
    status = await conn.fetchval("UPDATE TABLE user_session SET is_active = true WHERE user_id = $1 RETURNING id",user_id)
    return str(id)
