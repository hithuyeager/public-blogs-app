...
async def delete_user_blog(conn: asyncpg.Connection, user_id: UUID , blog_id: int) -> bool:
    is_deleted = await conn.fetchval("DELETE FROM users_blogs WHERE id = $2 AND user_id = $1 RETURNING true",user_id,blog_id)
